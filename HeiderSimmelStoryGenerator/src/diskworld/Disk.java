...
public class Disk implements CollidableObject {
	 ...
	 */
	private LinkedList<CollisionEventHandler> eventHandler;
	 ...
	 *            rotation angle of the disk
	 * @param zLevel
	 *            initial zLevel of the disk
	 * @param diskType
	 *            type of the disk
	 */

	protected Disk(DiskComplex diskComplex, double x, double y, double radius, double angle, int zLevel, DiskType diskType) {
		belongsTo = diskComplex;
		position = new RelativePosition(belongsTo.getCoordinates());
		position.setAbsPosition(x, y);
		position.setAbsAngle(angle);
	...
	/**
	 * Add an EventHandler (already existing EventHandler remain active!)
	 * 
	 * @param handler
	 *            new CollisionEventHandler
	 */
	public void addEventHandler(CollisionEventHandler handler) {
		if (eventHandler == null)
			eventHandler = new LinkedList<CollisionEventHandler>();
	 ...
	 * 
	 * @param handler
	 *            CollisionEventHandler to be removed
	 */
	public void removeEventHandler(CollisionEventHandler handler) {
		if (eventHandler != null)
			eventHandler.remove(handler);
	}

	/**
	 * Get the currently installed EventHandlers
	 * 
	 * @return list of current EventHandlers or null
	 */
	public LinkedList<CollisionEventHandler> getEventHandler() {
		return eventHandler;
	}
	...
	public void callEventHandler(CollidableObject obj2, double px, double py, double exchangedImpulse) {
		if (eventHandler != null)
			for (CollisionEventHandler eh : eventHandler)
				eh.collision(obj2, new Point(px, py), exchangedImpulse);
	}

	/**
	 * Convenience method: Creates a new disk neighboring to this and adds it to the same diskComplex
	 * 
	 * @param relativeAngle
	 ...
	 * @return the newly created disk
	 */
	public Disk attachDisk(double relativeAngle, double radius, double orientationOffset, int zLevel, DiskType diskType) {
		double direction = getAngle() + relativeAngle;
		double sr = getRadius() + radius;
		return belongsTo.addNewDisk(getX() + sr * Math.cos(direction), getY() + sr * Math.sin(direction), radius, direction + orientationOffset, zLevel, diskType, this);
	}
	...
	public Joint attachJoint(double relativeAngle, double radius, double orientationOffset, DiskType diskType) {
		double direction = getAngle() + relativeAngle;
		double sr = getRadius() + radius;
		return Joint.createJoint(belongsTo, this, getX() + sr * Math.cos(direction), getY() + sr * Math.sin(direction), radius, direction + orientationOffset, diskType);
	}
	 ...
	 * @return the newly created disk
	 */
	public Joint attachJoint(double relativeAngle, double radius, double orientationOffset, int zLevel, DiskType diskType) {
		double direction = getAngle() + relativeAngle;
		double sr = getRadius() + radius;
		return Joint.createJoint(belongsTo, this, getX() + sr * Math.cos(direction), getY() + sr * Math.sin(direction), radius, direction + orientationOffset, zLevel, diskType);
	}
	 ...
	 */
	public double getEnergyConsumedByActuatorAndSet0() {
		double res = consumedEnergyByActuator;
		consumedEnergyByActuator = 0;
		return res;
	}
	 ...
	 */
	public void incEnergyConsumedByActuator(double energyConsumed) {
