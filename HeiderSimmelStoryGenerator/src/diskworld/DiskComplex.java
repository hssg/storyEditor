...
public class DiskComplex implements Serializable {
	...
	Disk addNewDisk(double x, double y, double radius, double angle, int zLevel, DiskType diskType, Disk connectedTo) {
		Disk disk = new Disk(this, x, y, radius, angle, zLevel, diskType);
		addNewDisk(disk, connectedTo, false);
		return disk;
	}

	public void addNewDisk(Disk newDisk, Disk connectedTo, boolean isControllable) {
		if (connectedTo == null)
	...
	void applyFriction(double dt, FrictionModel frictionModel) {
	...
	public void applyGravity(double dt, GravityModel gravityModel) {
		if (isAffectedByGravity) {
			applyAllGravityImpulses(dt, gravityModel);
			reCalculateSpeeds();
		}
	}
	...
	private void applyAllGravityImpulses(double dt, GravityModel gravityModel) {
		for (Disk d : disks) {
			applyGravityImpulse(d, dt, gravityModel);
		}
	}

	private void applyGravityImpulse(Disk d, double dt, GravityModel gravityModel) {
		double force[] = gravityModel.getGravityForce(d);
		double deltamx = dt * force[0];
		double deltamy = dt * force[1];
		applyImpulse(deltamx, deltamy, d.getX(), d.getY());
	}
	...
	private void performMerge(DiskComplex other) {
		...
		diskcomplexEnsemble.removeDiskComplex(other);
	}
	...
	public void setDiskFixed(int index) {
		fixedDisks.add(disks.get(index));
	}
	...
	public boolean tryPerformDiskModifications(Collection<DiskModification> diskModifications, CollisionDetector collisionDetector, double timeStep, CollisionTracker collisionTracker,
		...
		// store lengths of transient edges, to keep track of loosened connections
		skeleton.storeTransientEdgesRelativeLengths();
		...
		if (collisions.isEmpty()) {
			...
			if (skeleton.diskModificationsSplitSkeleton()) {
	...
	/**
	 * Split from all neighbors of a disk.
	 * 
	 * @param disk
	 *            disk to be isolated
	 */
	public void splitFromNeighbors(Disk disk) {
		boolean split = false;
		for (Disk neighbor : disk.getSkeletonVertex().getNeighborDisks()) {
			if (skeleton.removalOfPermanentEdgeSplitsSkeleton(disk, neighbor)) {
				split = true;
			}
		}
		if (split) {
	...
	public void setAngularSpeed(double newAngularSpeed) {
		angularMomentum = newAngularSpeed * massMomentum;
		angularSpeed = angularMomentum / massMomentum;
	}
	...
	public void addSkeletonEdge(Disk d1, Disk d2) {
		skeleton.addPermanentEdge(d1, d2);
	}

	public void setAffectedByGravity(boolean gravityOn) {
		this.isAffectedByGravity = gravityOn; 
	}
