 ...
 * 
 */
public class DiskComplexEnsemble implements Serializable {
	 ...
	 */
	public void doTimeStep(double dt, PhysicsParameters physicsParameters, CollisionDetector collisionDetector, CollisionTracker collisionTracker, DiskChangeListener collisionDetectorChangeListener) {
		FrictionModel fm = physicsParameters.getFrictionModel();
		GravityModel gm = physicsParameters.getGravityModel();
		for (DiskComplex dc : diskComplexes) {
			if (fm != null) {
				if (dc.isMoving()) {
					dc.applyFriction(dt, fm);
				}
			}
			if (gm != null) {
				dc.applyGravity(dt, gm);
			}
			if (dc.isFixed()) {
				dc.setVelocity(0, 0);
				dc.setAngularSpeed(0);
			}
			dc.doTimeStep(dt, collisionDetector, collisionTracker, collisionDetectorChangeListener, changeListeners);
		}
	}
