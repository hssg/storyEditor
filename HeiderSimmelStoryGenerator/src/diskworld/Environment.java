...
import diskworld.environment.StokesFrictionModel;
...
public class Environment {
	...
	private static final boolean DEFAULT_USE_GRID_BASED_COLLISION_DETECTION = false;
	...
	private static Collection<Wall> getBoundingWalls(double maxx, double maxy) {
		return getBoundingWalls(maxx, maxy, DEFAULT_WALL_THICKNESS);
	}

	private static Collection<Wall> getBoundingWalls(double maxx, double maxy, double thickness) {
		LinkedList<Wall> res = new LinkedList<Wall>();
		double x1 = thickness * 0.5;
		double y1 = thickness * 0.5;
		double x2 = maxx - thickness * 0.5;
		double y2 = maxy - thickness * 0.5;
		res.add(new Wall(new Line(new Point(x1, y1), new Point(x2, y1)), thickness));
		res.add(new Wall(new Line(new Point(x1, y2), new Point(x2, y2)), thickness));
		res.add(new Wall(new Line(new Point(x1, y1), new Point(x1, y2)), thickness));
		res.add(new Wall(new Line(new Point(x2, y1), new Point(x2, y2)), thickness));
		return res;
	}
	...
	public Joint newRootJoint(double x, double y, double radius, double angle, DiskType diskType, boolean fixed) {
		...
		Joint joint = Joint.createJoint(dc, null, x, y, radius, angle, diskType);
		if (fixed)
	...
	public boolean canChangeIsolatedDiskRadius(Disk d, double newRadius) {
		DiskComplex dc = d.getDiskComplex();
		if (dc.getDisks().size() > 1) {
			// not possible for non-isolated disks
			return false;
		}
		double oldRadius = d.getRadius();
		d.setRadius(newRadius);
		if (collisionDetector.getCollisions(dc).isEmpty()) {
			return true;
		} else {
			// there is a collision created by this, undo and return false
			d.setRadius(oldRadius);
			return false;
		}		
	}
	...
	public synchronized void doTimeStep(double dt, AgentMapping[] mappings) {
		...
		if (physicsParameters.getGravityModel() != null)
			physicsParameters.getGravityModel().nextTimeStep(dt);
		while (dt > 0.0) {
			...
			diskComplexes.doTimeStep(partial_dt, physicsParameters, collisionDetector, collisionTracker, collisionDetectorChangeListener);
	...
	private void performActuatorEffects(double partial_dt, double dt, boolean firstSlice) {
		for (Disk d : diskComplexes.getActuatorDisks()) {
			...
			d.incEnergyConsumedByActuator(energyConsumed);
		}
	}
	...
	/**
	 * Removes a disk.
	 * The disk is removed from the DiskComplex it belongs to and all skeleton edges to this disk are removed.
	 * This may split the DiskComplex into multiple pieces.
	 * If the DiskComplex contains only one disk, it gets removed from the DiskComplexEnsemble.
	 */
	public void deleteDisk(Disk d) {
		DiskComplex dc = d.getDiskComplex();
		dc.splitFromNeighbors(d);
		dc = d.getDiskComplex(); // disk complex might have changed due to splitting
		diskComplexes.removeDiskComplex(dc);
	}

	public void merge(Disk disk, Disk otherDisk) {
		DiskComplex dc1 = disk.getDiskComplex();
		DiskComplex dc2 = otherDisk.getDiskComplex();
		if (dc1 != dc2)
			dc1.mergePermanently(dc2, disk, otherDisk);
	}

	public void setStokesFriction() {
		physicsParameters.setFrictionModel(new StokesFrictionModel(floor, floorGrid));
	}
