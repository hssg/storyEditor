...
public class ObjectConstructor {
	...
	private static class DescriptionItem {
		...
		void createDisk(ArrayList<Disk> disks, double radiusFactor) {
			...
			if (diskType.isJoint()) {
				Joint joint = parent.attachJoint(positionAngle, radiusFactor * radius, rotationAngle, zLevel, diskType);
				for (Entry<ActionType, double[]> e : jointActionRange.entrySet()) {
					joint.setRange(e.getKey(), e.getValue());
				}
				disks.add(joint);
			} else {
				disks.add(parent.attachDisk(positionAngle, radiusFactor * radius, rotationAngle, zLevel, diskType));
			}
		}
	...
	public void createDiskType(String name, DiskMaterial material, Actuator actuator, Sensor[] sensors, JointActionType[] jointActions) {
		...
		if (jointActions != null) {
			for (JointActionType jointAction : jointActions) {
				dt.addJointAction(jointAction);
			}
		}
	...
	public void createDiskType(String name, DiskMaterial material, Actuator actuator, Sensor[] sensors) {
		createDiskType(name, material, actuator, sensors, null);		
	}

	public void createJointType(String name, DiskMaterial material, JointActionType[] jointActions) {
		createDiskType(name, material, null, null, jointActions);		
	}

	public void createJointType(String name, DiskMaterial material, JointActionType jointAction) {
		createDiskType(name, material, null, null, new JointActionType[] { jointAction });		
	}
	...
	public void setRoot(double rootRadius, boolean fixed, DiskType rootType) {
		rootDescription = new DescriptionItem(0, 0, 0, rootRadius, 0, rootType);
		fixedRoot = fixed;
	}
	...
	public int addItem(int parentIndex, double positionAngle, double rotationAngle, double radius, String diskTypeName) {
		childrenDescriptions.add(new DescriptionItem(parentIndex, positionAngle, rotationAngle, radius, 0, getType(diskTypeName)));
		return childrenDescriptions.size();
	}
	
	public int addItem(int parentIndex, double positionAngle, double rotationAngle, double radius, int zLevel, String diskTypeName) {
		childrenDescriptions.add(new DescriptionItem(parentIndex, positionAngle, rotationAngle, radius, zLevel, getType(diskTypeName)));
		return childrenDescriptions.size();
	}
	...
	public int addItem(int parentIndex, double positionAngle, double rotationAngle, double radius, DiskType diskType) {
		childrenDescriptions.add(new DescriptionItem(parentIndex, positionAngle, rotationAngle, radius, 0, diskType));
		return childrenDescriptions.size();
	}

	public int addItem(int parentIndex, double positionAngle, double rotationAngle, double radius, int zLevel, DiskType diskType) {
		childrenDescriptions.add(new DescriptionItem(parentIndex, positionAngle, rotationAngle, radius, zLevel, diskType));
		return childrenDescriptions.size();
	}
