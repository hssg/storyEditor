...
public class ActuatorBag implements Actuator {
	 ...
	 * 
	 * @param actuatorInBagIndex
	 *            index of the target actuator in the bag
	 * @param componentOfActuatorValueIndex
	 *            index of the component of the target value (in the actuators own counting scheme, i.e. 0 = first activity value)
	...
	public double evaluateEffect(Disk disk, Environment environment, double[] activity, double partial_dt, double total_dt, boolean firstSlice) {
		...
		for (Actuator c : components) {
			double[] subArray = Arrays.copyOfRange(activity, index, index + c.getDim());
			sum += c.evaluateEffect(disk, environment, subArray, partial_dt, total_dt, firstSlice);
			index += c.getDim();
		}
		return sum;
	}
