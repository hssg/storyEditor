...
public class Mover extends ActuatorWithVisualisation {
	...
	private static final AbstractDiskSymbol DISK_SYMBOL = PolygonDiskSymbol.getSharpTriangleSymbol(0.7);
	...
	public double evaluateEffect(Disk disk, Environment environment, double[] activity, double partial_dt, double total_dt, boolean firstSlice) {
		if (firstSlice) {
			...
			return (Math.abs(rotationSpeed) * rotationEnergyConsumption * disk.getDiskComplex().getMassMomentum()
					+ Math.abs(translationSpeed) * moveEnergyConsumption * disk.getDiskComplex().getMass()) * total_dt;
		} else {
