...
public class Collision {
	...
	private final double rx2, ry2; // distance from object 2 center to collision point 
	...
	public void computeCollisionImpulses(double elasticity, double impulses[]) {
		...
		double speedDifference = computeSpeedDifference(dc1, dc2);
		if (speedDifference > Epsilons.EPSILON_SPEED_DIFF) {
			...
			double impulse = -speedDifference / (change1 + change2) * (1.0 + elasticity);
	...
	private double computeSpeedDifference(DiskComplex dc1, DiskComplex dc2) {
		...
		if (dc2 != null) {
			...
			v2y = dc2.getSpeedyRelative(rx2) + d2.getEgoMotiony();
		} else {
			v2x = 0;
			v2y = 0;
		}
		...
		return projspeed1 - projspeed2;
	}
	
	public double distanceSumSqr() {
