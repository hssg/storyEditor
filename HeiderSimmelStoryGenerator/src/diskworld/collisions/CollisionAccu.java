...
class CollisionAccu {
	...
	public void exchangeImpulse(Disk obj1, CollidableObject obj2, PhysicsParameters param) {
		...
		double sumy = 0.0;
		int num = 0;
		//if (!collisions.entrySet().isEmpty())
		//System.out.println("DEBUG: Collision accu has " + collisions.entrySet().size());
		for (Entry<Collision, Integer> e : collisions.entrySet()) {
			Collision c = e.getKey();
			int sign = e.getValue();
			c.computeCollisionImpulses(elasticity, impulses);
			//System.out.println("DEBUG: Collision accu impulses: " + impulses[0] + "," + impulses[1] + "," + impulses[2] + "," + impulses[3]);
