...
public class CollisionTracker {
	...
	private List<Point> blockedCollisions;
	private List<Point> unblockedCollisions;
	...
	public CollisionTracker(PhysicsParameters physicsParameters) {
		...
		blockedCollisions = new LinkedList<Point>();
		unblockedCollisions = new LinkedList<Point>();
		this.physicsParameters = physicsParameters;
	}

	public void clear() {
		storeCollisionPoints();
		map.clear();
		mergeMap.clear();
	}
	...
	private void storeCollisionPoints() {
		map.doForAll(new ElementMethod<CollidableObject, CollisionAccu>() {
			...
			public void forEach(CollidableObject key1, CollidableObject key2, CollisionAccu value) {
				if (value.isBlocked())
					blockedCollisions.add(value.getCollisionPoint());
				else
					unblockedCollisions.add(value.getCollisionPoint());
			}
		});
	}
	...
	public List<Point> getCollisionPoints(final boolean blocked) {
		List<Point> res;
		if (blocked) {
			res = blockedCollisions;
			blockedCollisions = new LinkedList<Point>();
		} else {
			res = unblockedCollisions;
			unblockedCollisions = new LinkedList<Point>();
		}
		return res;
	}
	...
	public void checkMerging() {
		// TODO: make deterministic, provide deterministic hash value
