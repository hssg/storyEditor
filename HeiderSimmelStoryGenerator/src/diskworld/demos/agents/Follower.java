...
public class Follower extends AgentDemo {

	private static final double SENSOR_OPENING_ANGLE = Math.toRadians(240);
	private static final double MAX_DISPLACEMENT_DISTANCE = 0.5;
	...
	public Environment getEnvironment() {
		...
		env.getFloor().fill(FloorCellType.WATER);
		...
		mover = new Mover(0, MAX_DISPLACEMENT_DISTANCE, MAX_ROTATION, 0.1, 0.1);
	...
	public boolean adaptVisualisationSettings(VisualizationSettings settings) {
		...
		//settings.getOptions().getOption(VisualizationOptions.GROUP_GENERAL, VisualizationOptions.OPTION_DISK_SYMBOLS).setEnabled(false);
		VisualizationOption opt = settings.getOptions().getOption(VisualizationOptions.GROUP_SENSORS, ClosestDiskSensor.SENSOR_NAME);
		if (opt != null) {
			opt.setChosenOption(0);
			return true;
		}
		return false;
	}
