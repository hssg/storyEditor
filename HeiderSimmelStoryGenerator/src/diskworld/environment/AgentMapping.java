...
public class AgentMapping {
	...
	/**
	 * Convenience constructor: constructs mapping using multiple diskComplexes in given order
	 * 
	 * @param diskComplexes
	 *            all disk actions, actuators and sensors of the disk complexes this list will be added to the mapping (in same order)
	 */
	public AgentMapping(DiskComplex[] diskComplexes) {
		this();
		for (DiskComplex dc : diskComplexes) {
			addJointActions(dc);
			addActuatorActions(dc);
			addAllSensorValues(dc);
		}
	}
