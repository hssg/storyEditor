...
public class ClosestDiskSensor extends ConeSensor {
	...
	private int offset, posIndex, distIndex, widthIndex, materialIndex;
	...
	public ClosestDiskSensor(
			...
			int maxNumDisks) {
		super(environment, centerAngle, openingAngle, minRangeRelative, maxRangeAbsolute, maxNumDisks, SENSOR_NAME);
	...
	public int getDimension() {
		return 1 + offset * maxNumDetections;
	}
	...
	protected void doMeasurement(double measurement[]) {
		...
		DiskData[] closestDisks = determineClosestDisks(maxNumDetections);
		measurement[0] = (double) closestDisks.length / (double) maxNumDetections;
	...
	public String getRealWorldMeaning(int index) {
		...
		int diskIndex = (index - 1) / offset;
		int modindex = index - diskIndex * offset;
		String diskStr = maxNumDetections > 1 ? " of disk #" + diskIndex : "";
		if (modindex == distIndex) {
