...
public abstract class ShapeBasedSensor extends AbstractSensor {
	...
	private AtomicReference<ShapeTemplate> sensorShapeTemplate;
	...
	public ShapeBasedSensor(Environment environment, ShapeTemplate sensorShape, String sensorName) {
		...
		sensorShapeTemplate = new AtomicReference<ShapeTemplate>(sensorShape);
	...
	public void doMeasurement(Disk disk, double[] values) {
		sensorShape = sensorShapeTemplate.get().getShape(disk.getX(), disk.getY(), disk.getRadius(), disk.getAngle());
		this.disk = disk;
	...
	/**
	 * Set a new sensor shape 
	 * 
	 * @param sensorShape the new template for sensor shapes
	 */
	protected void setShapeTemplate(ShapeTemplate sensorShape) {
		sensorShapeTemplate.set(sensorShape);
	}
			...
			public void draw(Graphics2D g, double centerx, double centery, double radius, double angle, double[] activity, double measurement[], VisualizationSettings settings, DiskType diskType) {
				...
				if (visualisation != null) {
					...
					if (visualisation instanceof ShapeVisualisationVariant) {
						shape = sensorShapeTemplate.get().getShape(centerx, centery, radius, angle);
